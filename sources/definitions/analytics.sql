...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_ENRICHED
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_GROUPS
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Entity resolution with blocking (BLOCK-01..04). Threshold >= 0.70.'
AS
WITH base AS (
    SELECT DISTINCT
    ...
    FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_ENRICHED e
...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_GOLDEN
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Golden customer records with survivorship + DQ scoring. ALL versions for SCD2.'
AS
WITH grouped AS (
    ...
    FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_GROUPS g
    JOIN {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_ENRICHED u ON g.source_system = u.source_system AND g.source_key = u.source_key
),
survivorship AS (
...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Current golden customer records (latest version only).'
AS
SELECT customer_id, first_name, last_name, email, phone, dq_score, source_count, row_timestamp AS last_updated
FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_GOLDEN
QUALIFY ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY file_date DESC, row_timestamp DESC) = 1;

DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_HISTORY
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'SCD Type 2 customer history. Declarative via SHA2 row-hash + LAG.'
AS
WITH versioned AS (
...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_GROUPS
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Links source addresses to master customer records. 1:1 customer-to-address.'
AS
WITH linked AS (
    ...
    JOIN {{db}}.{{agg_schema}}.CRMA_AGG_DT_CUSTOMER_GROUPS g ON a.source_system = g.source_system AND a.source_customer_key = g.source_key
)
SELECT customer_id AS address_id, customer_id, source_system, source_key, customer_id::VARCHAR AS cluster_id
FROM linked;

DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_GOLDEN
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Golden address per customer with survivorship. ALL versions for SCD2.'
AS
WITH grouped AS (
    SELECT g.address_id, g.customer_id, g.source_system, g.source_key,
        ...
        CASE g.source_system WHEN 'CRM_A' THEN 1 WHEN 'CRM_B' THEN 2 ELSE 3 END AS source_priority
    FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_GROUPS g
...
DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'Current golden address per customer (latest version only).'
AS
SELECT address_id, customer_id, address_type, street, city, postal_code, country, is_primary
FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_GOLDEN
QUALIFY ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY file_date DESC, row_timestamp DESC) = 1;

DEFINE DYNAMIC TABLE {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_HISTORY
    WAREHOUSE = {{warehouse}}
    TARGET_LAG = '{{dt_lag}}'
    REFRESH_MODE = FULL
    COMMENT = 'SCD Type 2 address history. Declarative via SHA2 row-hash + LAG.'
AS
WITH versioned AS (
    SELECT address_id, customer_id, address_type, street, city, postal_code, country, is_primary, file_date, row_timestamp,
        SHA2(CONCAT(COALESCE(address_type,''),'|',COALESCE(street,''),'|',COALESCE(city,''),'|',COALESCE(postal_code,''),'|',COALESCE(country,''),'|',COALESCE(is_primary::VARCHAR,''))) AS row_hash,
        LAG(SHA2(CONCAT(COALESCE(address_type,''),'|',COALESCE(street,''),'|',COALESCE(city,''),'|',COALESCE(postal_code,''),'|',COALESCE(country,''),'|',COALESCE(is_primary::VARCHAR,'')))) OVER (PARTITION BY customer_id ORDER BY file_date, row_timestamp) AS prev_hash
    FROM {{db}}.{{agg_schema}}.CRMA_AGG_DT_ADDRESSES_GOLDEN
),
changes AS (SELECT * FROM versioned WHERE prev_hash IS NULL OR row_hash != prev_hash),
scd2 AS (
...
SELECT address_id, customer_id, address_type, street, city, postal_code, country, is_primary, valid_from, valid_to,
    CASE WHEN valid_to = '9999-12-31'::TIMESTAMP_LTZ THEN TRUE ELSE FALSE END AS is_valid, row_hash
FROM scd2;
